...
var (
	...
	memo           = make(map[int]map[uint64]map[uint64]int) // map[ time/mins remaining int ]map[ int id of valve ]map[ bit map of valves ] int score
	vIds           = NewValveIDMap()
...
func main() {
	...
	pressure := getPressureFrom(valvesMap, opened, vIds.getInt("AA"), 30)

	fmt.Println(pressure)

}

func getPressureFrom(valvesMap ValveMap, opened OpenValveMap, valveId uint64, timeRemaining int) int {

	if timeRemaining < 1 {
		return 0
	}

	if _, e := memo[timeRemaining]; !e {
		memo[timeRemaining] = make(map[uint64]map[uint64]int)
	}
	if _, e := memo[timeRemaining][valveId]; !e {
		memo[timeRemaining][valveId] = make(map[uint64]int)
	}

	if v, e := memo[timeRemaining][valveId][uint64(opened)]; e {
		return v
	}
	...
	pressureThisMin := 0
	for vId, v := range possibleToOpen {
		if opened.isOpen(vId) {
			pressureThisMin += v.rate
		}
	}

	canOpenThisValve := false
	openedWithThis := opened.open(valveId)
	if !opened.isOpen(valveId) && valvesMap[valveId].rate > 0 {
		canOpenThisValve = true
	}

	if canOpenThisValve {
		maxPressure = getPressureFrom(valvesMap, openedWithThis, valveId, timeRemaining-1)
	}

	for vId := range valvesMap[valveId].to {
		var pressure int
		pressure = getPressureFrom(valvesMap, opened, vId, timeRemaining-1)
		if pressure > maxPressure {
			maxPressure = pressure
		}
	}

	maxPressure = pressureThisMin + maxPressure

	memo[timeRemaining][valveId][uint64(opened)] = maxPressure
