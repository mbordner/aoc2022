...
func main() {
	...
	if m != nil {
		...
		stepsTaken := 0
		path := m.StepsToGoal(m.start, m.goal, stepsTaken, 500)
		if path != nil {
			stepsTaken += len(path) - 1
			clearMemo()
			path = m.StepsToGoal(m.goal, m.start, stepsTaken, 1000)
			if path != nil {
				stepsTaken += len(path) - 1
				clearMemo()
				path = m.StepsToGoal(m.start, m.goal, stepsTaken, 1500)
				stepsTaken += len(path) - 1
